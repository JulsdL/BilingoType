...
[![Downloads](https://img.shields.io/github/downloads/JulsdL/BilingoType/total?style=flat&color=blue)](https://github.com/JulsdL/BilingoType/releases)
...
[![Star History Chart](https://api.star-history.com/svg?repos=JulsdL/BilingoType&type=date&legend=top-left)](https://www.star-history.com/#JulsdL/BilingoType&type=date&legend=top-left)
...
- ☁️ **BilingoType Cloud**: Sign in and transcribe instantly — no API keys needed, with free and Pro plans
...
1. **Clone the repository**:
   ...
   ```bash
   git clone https://github.com/JulsdL/BilingoType.git
   cd bilingotype
   ```
...
# The unsigned app will be in: dist/mac-arm64/BilingoType.app (macOS)
# or dist/win-unpacked/BilingoType.exe (Windows)
# or dist/linux-unpacked/bilingotype (Linux)
```
...
BilingoType ships a native C binary for pasting text on Windows, using the Win32 `SendInput` API. This is the **primary** paste mechanism — `nircmd` and PowerShell are only used as fallbacks if the native binary fails.
...
BilingoType now supports multiple Linux package formats for maximum compatibility:
...
# - BilingoType-x.x.x-linux-x64.AppImage
# - BilingoType-x.x.x-linux-x64.deb
# - BilingoType-x.x.x-linux-x64.rpm
# - BilingoType-x.x.x-linux-x64.tar.gz
```
...
```bash
# Debian/Ubuntu
sudo apt install ./dist/BilingoType-*-linux-x64.deb

# Fedora/RHEL
sudo dnf install ./dist/BilingoType-*-linux-x64.rpm

# Universal tar.gz (no root required)
tar -xzf dist/BilingoType-*-linux-x64.tar.gz
cd BilingoType-*/
./bilingotype

# Flatpak
flatpak install --user ./dist/BilingoType-*-linux-x64.flatpak

# AppImage (existing method)
chmod +x dist/BilingoType-*.AppImage
./dist/BilingoType-*.AppImage
```

**Native Paste Binary (`linux-fast-paste`)**:

BilingoType ships a native C binary for pasting text on Linux, compiled automatically at build time. This is the **primary** paste mechanism — external tools like `xdotool` and `wtype` are only used as fallbacks if the native binary fails.
...
If the native binary isn't available, BilingoType falls back to external paste tools in this order:
...
> ℹ️ **Note**: BilingoType automatically tries paste methods in this order: native `linux-fast-paste` binary (XTest or uinput) → `wtype` → `ydotool` → `xdotool` (for XWayland apps). If no paste method works, text will still be copied to the clipboard - you'll just need to paste manually with Ctrl+V.
...
On GNOME Wayland, Electron's standard global shortcuts don't work due to Wayland's security model. BilingoType automatically uses native GNOME keyboard shortcuts via D-Bus and gsettings:
...
1. **Choose Processing Method**:
   - **BilingoType Cloud**: Sign in for instant cloud transcription with free and Pro plans
...
- **In-App**: Use _Settings → General → Local Model Storage → Remove Downloaded Models_ to clear `~/.cache/bilingotype/whisper-models` (or `%USERPROFILE%\.cache\bilingotype\whisper-models` on Windows).
...
- **macOS**: If you uninstall manually, remove `~/Library/Caches` or `~/.cache/bilingotype/whisper-models` if desired.
...
- Brand names (e.g., "BilingoType", "whisper.cpp")
...
For local processing, BilingoType uses OpenAI's Whisper model via whisper.cpp - a high-performance C++ implementation:

1. **Bundled Binary**: whisper.cpp is bundled with the app for all platforms
2. **GGML Models**: Downloads optimized GGML models on first use to `~/.cache/bilingotype/whisper-models/`
...
**Upgrading from Python-based version**: If you previously used the Python-based Whisper, you'll need to re-download models in GGML format. You can safely delete the old Python environment (`~/.bilingotype/python/`) and PyTorch models (`~/.cache/whisper/`) to reclaim disk space.

### Local Parakeet Setup (Alternative)

BilingoType also supports NVIDIA Parakeet models via sherpa-onnx - a fast alternative to Whisper:

1. **Bundled Binary**: sherpa-onnx is bundled with the app for all platforms
2. **INT8 Quantized Models**: Efficient CPU inference
3. **Models stored in**: `~/.cache/bilingotype/parakeet-models/`
...
BilingoType is designed with privacy and security in mind:
...
2. **Accessibility permissions (macOS)**: Required for automatic text pasting
   ...
   - Add BilingoType and enable the checkbox
...
- Check the [Issues](https://github.com/JulsdL/BilingoType/issues) page
...
**Q: Is BilingoType really free?**
A: Yes! BilingoType is open source and free to use. The free plan includes 2,000 words/week of cloud transcription, and local processing is completely free with no limits. Pro plan ($9/month) offers unlimited cloud transcription.
...
A: BilingoType supports 58 languages including English, Spanish, French, German, Chinese, Japanese, and more. Set your preferred language in the .env file or use auto-detect.

## Project Status

BilingoType is actively maintained and ready for production use. Current version: 1.5.4

- ✅ Core functionality complete
- ✅ Cross-platform support (macOS, Windows, Linux)
- ✅ BilingoType Cloud with account system and usage tracking
