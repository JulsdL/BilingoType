...
export default function App() {
  ...
  useEffect(() => {
    const resizeWindow = () => {
      ...
      } else if (isStreaming && partialTranscript) {
        window.electronAPI?.resizeMainWindow?.("WITH_PARTIAL");
      } else {
        window.electronAPI?.resizeMainWindow?.("BASE");
      }
    };
    resizeWindow();
  }, [isCommandMenuOpen, toastCount, isStreaming, partialTranscript]);
      ...
      <div className="fixed bottom-6 right-6 z-50">
        ...
        >
          ...
          {isStreaming && partialTranscript && (
            <div className="absolute bottom-full right-0 mb-2 max-w-[260px] px-2.5 py-1.5 rounded-lg bg-popover/95 border border-border shadow-lg backdrop-blur-sm pointer-events-none">
              <p className="text-xs text-popover-foreground line-clamp-2 leading-relaxed">
                {partialTranscript}
              </p>
            </div>
          )}
